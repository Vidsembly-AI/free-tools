...
import { isVideoFile } from "@/lib/tools/video/is-video-file";
import { convertVideoToMp3 } from "@/lib/tools/video/video-to-mp3";
...
export function VideoToMp3Converter({ trustPoints }: VideoToMp3ConverterProps) {
  ...
  const addFiles = useCallback((incoming: File[]) => {
    const videoFiles = incoming.filter(isVideoFile);
    const rejectedCount = incoming.length - videoFiles.length;

    if (videoFiles.length === 0) {
      setStatusVariant("error");
      setStatusMessage("Please upload valid video files only.");
      return;
    }
    ...
    if (rejectedCount > 0) {
      setStatusVariant("error");
      setStatusMessage(`${rejectedCount} file(s) skipped — only video files are supported.`);
    } else {
      setStatusVariant("info");
      setStatusMessage(`Added ${videoFiles.length} video file${videoFiles.length === 1 ? "" : "s"}.`);
    }
  }, []);
  ...
  const convertAll = useCallback(async () => {
    ...
    for (const item of pending) {
      ...
      try {
        const result = await convertVideoToMp3(item.file, (message) => {
          setStatusVariant("loading");
    ...
    if (errorCount === 0) {
      ...
      setStatusVariant("error");
      setStatusMessage("All conversions failed. Please check your video files and try again.");
    } else {
      setStatusVariant("error");
      setStatusMessage(
  ...
  return (
    <div className="space-y-6">
      <FileDropzone
        accept=".mp4,.webm,.mov,.avi,.mkv,.m4v,.wmv,.flv,.ogv,.mpeg,.mpg,.3gp,video/*"
